...
export default function ProductCard({ item, onPress, itemWidth, currencySymbol }: ProductCardProps) {
    ...
    const isUnlimited = !firstVariant || !firstVariant.manageStock;
    const stockQty = firstVariant?.stocks?.[0]?.quantity || 0;
    ...
    return (
        <View style={{ width: `${itemWidth}%`, padding: 6 }}>
            <TouchableOpacity
                ...
                className="flex-row items-center p-3 bg-white border shadow-sm rounded-2xl border-slate-100"
            >
                    ...
                    <View className="flex-row items-center">
                        {isUnlimited ? (
                            <View className="flex-row items-center bg-emerald-50 px-1.5 py-0.5 rounded-md">
                                <InfinityIcon size={8} color="#10B981" />
                                <Text className="text-[8px] text-emerald-600 font-bold ml-1 uppercase italic">Stok: Aman</Text>
                            </View>
                        ) : (
                        ...
                        <Text className="ml-2 text-[10px] font-black text-indigo-600">
                            {currencySymbol} {Number(firstVariant?.price || 0).toLocaleString()}
                        </Text>
                    </View>
                </View>
            </TouchableOpacity>
